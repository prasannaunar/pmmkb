...
PMM terminology used throughout this knowledge base, plus the industry terms most entries assume you already know. Each term links back to the entry (or entries) where it is used in context; read the term here for a fast definition, then follow the link for the full application.

Terms are grouped by theme, then alphabetical within each group. See also [BY-SITUATION.md](BY-SITUATION.md) to go the other way: from a business situation to the relevant frameworks.
...
**MQL (Marketing-Qualified Lead):** A lead that marketing has judged ready to hand to sales, typically based on engagement signals like content downloads or webinar attendance.
...
**CAC (Customer Acquisition Cost):** The total sales and marketing cost to acquire one new customer, usually calculated as spend divided by new customers won in a period.

**CAC payback period:** The number of months of gross margin from a customer needed to recover the cost of acquiring them; a core efficiency metric in B2B SaaS.

**LTV (Lifetime Value):** The total gross margin a company expects to earn from a customer over the life of the relationship. Often compared against CAC as an LTV:CAC ratio.

**ARR (Annual Recurring Revenue):** The annualised value of recurring subscription revenue, the standard top-line metric for B2B SaaS growth tracking (see T2D3).

**MRR (Monthly Recurring Revenue):** The monthly equivalent of ARR; ARR divided by twelve.

**ACV (Annual Contract Value):** The average annualised value of a single customer contract, used to distinguish deal sizes across GTM motions (self-serve deals typically have low ACV; field sales deals typically have high ACV).

**NRR (Net Revenue Retention):** The percentage of recurring revenue retained from an existing customer cohort over a period, including expansion and net of churn and downgrades. Above 100% means expansion revenue outpaces churn.

**GRR (Gross Revenue Retention):** The same calculation as NRR but excluding expansion revenue, showing retention from churn and downgrades alone. Always at or below 100%.
...
Found a term this knowledge base uses without defining? Add it here in the relevant theme group, alphabetically, with a one-to-two sentence definition and a link back to the entry that uses it, following CLAUDE.md's writing standards (British English, no em dashes).
