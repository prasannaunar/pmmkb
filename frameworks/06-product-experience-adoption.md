...
1. Churn analysis shows a disproportionate number of cancellations happen within customers' first 90 days, and the team suspects new customers are getting stuck somewhere before reaching real value. Which tool should they use first?
   - **A.** Customer Onboarding Maturity Framework, mapping the milestone drop-off.
   - **B.** Time to Value Framework, defining and validating the activation event.
   - **C.** RFM Model, segmenting the base by recency and value.
   - **D.** Kano Model, classifying features by their satisfaction effect.
   ...
   *Why not B:* Time to Value defines and validates the specific proxy metric for "reaching value," but it doesn't audit where in a milestone sequence customers get stuck; Onboarding Maturity is the tool built to map milestones against drop-off data and churned-customer interviews.

   *Why not C:* RFM segments customers by behavioural value and engagement; it is built for an established base with usage history, not for diagnosing where new customers drop off during their first 90 days.
...
2. Win rates have been declining for two quarters, and it is unclear whether the cause is positioning, pricing, product gaps, or sales execution. Which methodology directly investigates this?
   - **A.** Win/Loss Analysis Framework, interviewing recent wins and losses.
   - **B.** NPS Framework, surveying existing customers' sentiment.
   - **C.** Time to Value Framework, measuring speed to first value.
   - **D.** Customer Onboarding Maturity Framework, auditing the post-purchase journey.
   ...
   *Why not B:* NPS does probe why customers feel the way they do, but only for existing customers after purchase; it can't explain why a prospect chose a competitor during the sales cycle, which is exactly what win/loss interviews uncover.
...
3. A product team is deciding which of fifteen roadmap candidates should headline the next launch and which should just be quietly shipped as bug-fix-level improvements. Which tool helps make that distinction?
   - **A.** Kano Model, classifying each candidate by its effect on satisfaction.
   - **B.** NPS Framework, surfacing feature requests in detractor verbatims.
   - **C.** RFM Model, segmenting customers by engagement and value.
   - **D.** Win/Loss Analysis Framework, explaining why deals were won or lost.
   ...
   *Why not B:* NPS verbatims can surface that customers want a feature, but sentiment comments don't classify what effect it would have on satisfaction if built; Kano's paired survey is what tells the team whether a candidate is a Delighter worth headlining or a Must-be that only needs defensive fixing.

   *Why not C:* RFM segments customers by recency, frequency, and monetary value; it says nothing about how a specific feature affects customer satisfaction or where it belongs in a launch narrative.

   *Why not D:* Win/loss analysis explains why deals were won or lost; it does not classify roadmap candidates by their satisfaction effect for launch-sequencing purposes.
...
4. Marketing can only ever find a customer reference "just in time" for a specific ask, and the same three accounts get approached every time because they're the ones everyone remembers. Which tool diagnoses this?
   - **A.** Forrester Customer Advocacy Model, diagnosing the programme's maturity stage.
   - **B.** RFM Model, identifying which customers are frequent and valuable.
   - **C.** Customer Onboarding Maturity Framework, mapping the early customer journey.
   - **D.** Kano Model, classifying features by their satisfaction effect.
   ...
   *Why not B:* RFM can supply candidate Champions for the advocate pool, but identifying who is behaviourally valuable is not the same as diagnosing or fixing the advocacy programme's own maturity stage.
...
5. A customer scored a 9 on last quarter's NPS survey, but usage data shows they haven't logged in for two months. Which tool specifically catches this kind of mismatch between stated sentiment and actual behaviour?
   - **A.** RFM Model, scoring behaviour rather than stated sentiment.
   - **B.** NPS Framework, the survey that produced the reassuring score.
   - **C.** Kano Model, classifying features by their satisfaction effect.
   - **D.** Win/Loss Analysis Framework, explaining sales-cycle outcomes.
...
6. A team has been optimising "time to first login" for two quarters with no effect on retention, and suspects they may be tracking the wrong proxy for when customers actually experience value. Which tool addresses this specific problem?
   - **A.** Time to Value Framework, validating the activation event as a true proxy.
   - **B.** Customer Onboarding Maturity Framework, tracking time-to-first-value as a milestone.
   - **C.** RFM Model, measuring ongoing behavioural engagement.
   - **D.** NPS Framework, measuring sentiment after the fact.
...
7. A company wants to know which existing customers most urgently need a proactive save-play right now, based on actual behaviour rather than survey sentiment. Which tool answers this?
   - **A.** RFM Model, flagging the At-risk segment from behavioural data.
   - **B.** NPS Framework, the survey sentiment this scenario sets aside.
   - **C.** Kano Model, classifying features by their satisfaction effect.
   - **D.** Win/Loss Analysis Framework, explaining sales-cycle outcomes.
   ...
   *Why not B:* NPS is exactly the survey-sentiment signal this scenario says to set aside; it flags who feels positively, not which accounts are behaviourally at risk right now, which is RFM's specific, real-time read.

   *Why not C:* Kano classifies product features by satisfaction effect; it has no mechanism for identifying which individual customer accounts are behaviourally at risk right now.

   *Why not D:* Win/loss analysis explains sales-cycle outcomes for prospective and closed deals; it does not identify at-risk accounts within the existing customer base in real time.
...
8. A team notices that customers can complete every onboarding milestone in the model's checklist without ever renewing, and wants a rigorously validated, retention-tested definition of what actually predicts renewal, rather than just pairing milestone completion with a retention number. Which tool provides that specific sub-metric validation?
   - **A.** Time to Value Framework, testing candidate definitions against renewal data.
   - **B.** Customer Onboarding Maturity Framework, the milestone checklist itself.
   - **C.** Forrester Customer Advocacy Model, diagnosing advocacy programme maturity.
   - **D.** RFM Model, tracking behavioural value across an established base.
...
9. A support ticket surge cites a capability the team assumed was a rarely-noticed nice-to-have, and separately, this same capability keeps coming up in NPS detractor comments as something customers now expect by default. Which tool's classification has most likely shifted?
   - **A.** Kano Model, reclassifying a feature as expectations rise.
   - **B.** NPS Framework, the detractor comments naming the gap.
   - **C.** RFM Model, tracking behavioural engagement and value.
   - **D.** Customer Onboarding Maturity Framework, mapping the early customer journey.
   ...
   *Why not B:* NPS detractor comments are the signal that a reclassification is overdue, but NPS itself doesn't classify a feature's satisfaction effect; Kano's paired survey is the tool that actually reclassifies it.

   *Why not C:* RFM tracks behavioural engagement and value, not how a specific feature affects satisfaction; it would not surface a feature's shifting classification.
...
10. A company's advocacy programme has a formal tiered ask menu and a named owner, but every request is still generated reactively whenever marketing has a specific need, and no one has checked whether reference-assisted deals actually close at a higher rate. Which tool diagnoses the missing piece of the programme's maturity?
    - **A.** Forrester Customer Advocacy Model, naming the missing Embedded-stage piece.
    - **B.** RFM Model, supplying candidate Champions for the advocate pool.
    - **C.** Kano Model, classifying features by their satisfaction effect.
    - **D.** Customer Onboarding Maturity Framework, mapping the early customer journey.
...
1. A team writes three different email sequences and adds an in-app tutorial to try to fix a milestone stuck at 20% completion. None of the three interventions move the number. What should the team conclude?
   - **A.** Escalate the issue as a likely product or UX problem.
   - **B.** Try one more, shorter intervention before escalating.
   - **C.** Lower the milestone's completion target and stop investigating.
   - **D.** Remove the milestone from the model entirely.

   **Correct answer: A.** Ignoring the product is a named pitfall: onboarding content cannot overcome bad product design. The framework's threshold is explicit: more than one failed content intervention at the same milestone is the signal to escalate a product or UX fix rather than write another tutorial.

   *Why not B:* This is the right eventual move, just delayed. Three interventions have already failed, well past the "more than one" threshold the framework sets for escalating; trying a fourth first ignores a signal that has already fired.
...
2. A company applies the same onboarding email sequence to both its $20-a-month self-serve customers and its enterprise accounts. Enterprise customers report feeling undersupported, while SMB customers report the emails feel excessive. What pitfall does this illustrate?
   - **A.** One-size-fits-all onboarding, using one sequence for different segments.
   - **B.** Ignoring the product, requiring a UX redesign applied equally to both segments.
   - **C.** Measuring the wrong metrics, so the fix is dropping email as a channel.
   - **D.** Standardising content this way is intentional and needs no changes at all.
   ...
   *Why not B:* Ignoring the product is a real pitfall elsewhere in this framework, but it describes a product or UX defect that content can't fix; nothing here suggests a design flaw, only that identical content was sent to segments with very different needs.

   *Why not C:* Measuring the wrong metrics describes pairing milestone completion with the wrong outcome; the problem here is that a single content approach was applied across two segments with fundamentally different expectations, which segmentation, not a metric change, would fix.
...
3. A customer completes every defined onboarding milestone within the target timeframe, but churns after two months without renewing. What does the framework suggest the team check?
   - **A.** Whether milestone completion actually correlates with retention.
   - **B.** Whether the milestones were defined too generously in the first place.
   - **C.** Whether to exclude the customer from all future NPS surveys.
   - **D.** Whether sales oversold the product during the initial sales conversation.
   ...
   *Why not D:* A sales oversell is a plausible contributing cause, but the framework's specific guidance for this pattern, milestones completed without retention, is to check whether the milestones themselves correlate with real outcomes; investigating the sales process first skips that recommended step.
...
4. Onboarding milestones were validated with customer interviews two years ago, and the product has since added several major features and moved upmarket into larger accounts. The team is still using the original milestone list unchanged. What does the framework recommend?
   - **A.** Re-validate the milestone list now, given how much has changed.
   - **B.** Continue using the original milestones without ever revisiting them.
   - **C.** Add new milestones on top of the old ones without pruning any.
   - **D.** Wait for a churn spike before revisiting the milestone list.
   ...
   *Why not C:* This is a step toward the right idea, updating the list, but layering new milestones onto stale ones without pruning produces a bloated, unfocused model; the framework calls for re-validating the whole list, not just adding to it.
...
5. Sales promises a fast, simple setup to close a deal, but the actual onboarding process for that customer type is known internally to take significant hands-on configuration. The customer becomes frustrated during onboarding. What does the framework identify as the root cause?
   - **A.** Misaligned expectations set during the sales process.
   - **B.** A pure product design flaw that only engineering can resolve.
   - **C.** An execution issue isolated to the customer success team.
   - **D.** An unavoidable cost of onboarding that needs no process change.
   ...
   *Why not C:* Customer success does have to manage the resulting frustration, but isolating the problem to their execution misses where it actually started, upstream, during the sales process; the fix requires cross-team alignment, not a customer-success-only correction.
...
1. An interviewer asks a buyer, "Why didn't you choose us because of our great support?" The buyer gives a vague, agreeable answer. What is the problem with this question?
   - **A.** It is a leading question that biases the buyer's answer.
   - **B.** There is no problem; it confirms what the team already suspects.
   - **C.** The question should have been sent as a written survey instead.
   - **D.** The issue is that the question was too long, not that it was leading.
   ...
   *Why not C:* The framework does favour a semi-structured interview over a written survey, but for a different reason, it lets the interviewer follow up on interesting answers. That format choice is not what makes this specific question flawed; the problem is the assumption embedded in its wording.
...
2. A team runs win/loss analysis but only interviews recent losses, reasoning that losses are more urgent to understand than wins. What is the risk in this approach?
   - **A.** Analysing losses but not wins skews the strategy toward defence.
   - **B.** This is fine; losses are always more informative than wins for improving win rate.
   - **C.** The risk is a smaller sample size, not the strategic direction of the findings.
   - **D.** Wins should be analysed only once every two years, unlike losses.
   ...
   *Why not B:* The framework treats wins and losses as equally valuable sources of insight; losses alone tell only half the story and risk over-indexing strategy on defence rather than reinforcing what is actually working.

   *Why not C:* There is a real risk here, but it isn't just about sample size; the deeper risk is that a losses-only view distorts the strategy's direction toward defence, missing what is actually driving wins, not merely trimming precision.
...
3. Win/loss interviews reveal that three recent losses mentioned "we didn't know you could do X," even though the product already had that capability. What does the framework's example suggest as the appropriate response, and how quickly?
   - **A.** Add the unsurfaced capability to the demo script within two weeks.
   - **B.** Treat this as unfixable, requiring a multi-quarter engineering change.
   - **C.** Set the finding aside until it reaches the five-loss pattern threshold.
   - **D.** Escalate it as a pricing issue, since unclear value always points to price.
   ...
   *Why not C:* The framework's five-or-more guidance is a general rule of thumb for spotting a pattern, not a hard floor; its own example treats three losses citing the same specific, clearly fixable issue as worth acting on regardless, rather than dismissed as below-threshold.
...
4. A win/loss analysis is completed with clear findings and recommendations, but three months later nothing has changed in messaging, pricing, or the sales playbook. What pitfall does this describe?
   - **A.** Not following up; commit publicly to updating strategy within two weeks.
   - **B.** Asking leading questions; re-run the interviews with a different interviewer.
   - **C.** Segmenting by decision-maker incorrectly; re-analyse by CFO, CTO, and CMO roles.
   - **D.** This is expected; win/loss findings typically take six months to act on.
   ...
   *Why not B:* Asking leading questions is a real pitfall, but it describes bias introduced during the interviews themselves; the scenario describes findings that were never acted on afterward, a follow-through failure, not an interview-technique one.

   *Why not C:* Decision-maker segmentation is a step within the analysis itself; the scenario describes action never being taken after the analysis was already completed, a separate, later-stage failure.
...
5. A win/loss analysis finds that 8 of 15 losses went to a competitor because "it's what our enterprise parent company mandates," a decision made above the buyer's authority. How does the framework's own example suggest this finding should be treated relative to a smaller pattern of losses tied to unclear messaging?
   - **A.** As a largely unavoidable pattern, unlike the smaller, fixable messaging losses.
   - **B.** As the single highest-priority fix, since it is the largest loss count in the sample.
   - **C.** As evidence the product itself is fundamentally uncompetitive.
   - **D.** As a reason to stop pursuing enterprise accounts with a parent company.
   ...
   *Why not B:* Loss count alone doesn't determine priority; the example explicitly treats the larger, mandate-driven count as lower priority for action, since it is not something messaging or product changes can influence, unlike the smaller, actionable messaging-gap pattern.
...
1. A company's overall NPS is 45, and leadership treats this as a healthy, unified signal across the whole customer base. A closer look shows mid-market scores 60 while SMB scores 15. What should the team do?
   - **A.** Analyse and act on the two segments separately.
   - **B.** Keep reporting only the blended 45 score going forward.
   - **C.** Deprioritise the SMB segment going forward entirely.
   - **D.** Re-run the survey with different wording until scores match.
   ...
   *Why not B:* Reporting is still happening, and NPS is still being tracked; the problem is scope. A single blended number buries exactly the segment-level divergence that needs a distinct intervention, rather than surfacing it.
...
2. A team is proud that its NPS rose from 45 to 47 this quarter and reports the improvement to leadership without mentioning any of the qualitative themes behind it. What pitfall does this reflect?
   - **A.** Obsessing over the number rather than the themes behind it.
   - **B.** No follow-up on the Detractor themes driving sentiment.
   - **C.** Assuming all Detractors will inevitably churn.
   - **D.** This is not a pitfall; the two-point gain is a complete report.
   ...
   *Why not B:* No follow-up is a real pitfall, but it describes failing to act on a specific Detractor complaint; this scenario is about reporting a number without any qualitative context, a distinct problem from unresolved outreach.
...
3. A long-tenured, heavy user of the product scores a 4 (a Detractor) on NPS, and customer success considers this account a near-certain churn risk and deprioritises any further engagement. What does the framework suggest about this assumption?
   - **A.** Not all Detractors are churn risks; segment by usage and tenure first.
   - **B.** Treat every Detractor as an imminent churn risk, regardless of tenure.
   - **C.** Exclude this account from all future NPS surveys.
   - **D.** Offer this account an immediate retention discount without further diagnosis.
   ...
   *Why not D:* Acting quickly is the right instinct, but jumping straight to a discount skips the step the framework calls for first: segmenting by usage and tenure to tell a genuine risk apart from an engaged critic who just needs to be heard.
...
4. A Detractor reports that support response times are too slow. Three survey cycles later, response times still have not improved, and the same complaint keeps recurring. What does the framework say should have happened?
   - **A.** An owner should have been assigned to the theme within one week.
   - **B.** Keep surveying the same customer each quarter and wait for sentiment to improve.
   - **C.** Reclassify the customer as a Promoter, given their repeat engagement.
   - **D.** Change the survey question to avoid triggering the same complaint again.
   ...
   *Why not B:* Repeatedly surveying without acting on the finding is exactly the "no follow-up" pattern the framework warns against; passive monitoring will not improve sentiment if the underlying issue is never addressed.
...
5. A team wants to report a rising NPS trend to leadership as evidence of a successful quarter but has no other business metric to pair it with. What does the framework suggest?
   - **A.** Pair the NPS trend with a leading business metric like renewal rate.
   - **B.** NPS alone is always sufficient evidence of business health.
   - **C.** Replace NPS entirely with a purely financial commercial metric instead.
   - **D.** Wait until next year to report NPS trends at all.
   ...
   *Why not C:* This takes the right instinct, connecting NPS to commercial reality, too far; the framework's guidance is to pair NPS with a financial metric, not to discard sentiment data in favour of financial metrics alone.
...
1. A team plans to headline its next launch with a feature that Kano classifies as Must-be. What does the model suggest about this plan?
   - **A.** Change the plan; resource the feature defensively but don't headline the launch with it.
   - **B.** Keep the plan; Must-be features are the most important category and should always headline launches.
   - **C.** Remove the feature from the roadmap entirely, since it generates no delight.
   - **D.** Use the classification for pricing tier decisions only, not for launch messaging.
   ...
   *Why not D:* This narrows the classification's use too far. Kano output does inform packaging tier decisions, but the model's own guidance applies it to launch messaging just as directly; the two uses aren't a choice between one or the other.
...
2. A feature that was a clear Delighter when first launched two years ago is now offered by every major competitor in the category. The team still treats it as a headline differentiator in current messaging. What does the model suggest?
   - **A.** Re-survey the feature rather than assume its old classification still holds.
   - **B.** The feature's classification is permanent, so messaging should stay unchanged.
   - **C.** Remove the feature from the product now that competitors also offer it.
   - **D.** Increase investment in the feature to restore Delighter status, without re-surveying first.
...
3. A team surveys its whole customer base together for a new feature and finds it lands as broadly "Indifferent." A closer look shows power users rate it a strong Delighter while new users are genuinely indifferent to it. What did the blended result miss?
   - **A.** Segment-level classification differences that averaging erased.
   - **B.** Nothing; the Indifferent classification is accurate once calculated.
   - **C.** The survey should have used a different five-point rating scale.
   - **D.** The feature should be reclassified as Reverse, given the mixed reactions.
...
5. A product team wants to know whether a proposed integration should be resourced at all, given limited engineering capacity, and is currently deciding based only on how many customers requested it in a support ticket. What would the Kano Model add to this decision that a simple request count does not capture?
   - **A.** Which of the five satisfaction effects the feature actually has.
   - **B.** Nothing; request volume is sufficient for a roadmap prioritisation decision.
   - **C.** The model only applies to features already built, not unbuilt ones.
   - **D.** Build it only if fewer than ten customers requested it, since Delighters must surprise.
...
2. An audit finds that four long-tenured customer accounts have absorbed 90% of all advocacy asks over two years, while a separate NPS analysis shows 62 other Promoters have never been approached. What stage does this pattern most likely indicate, and what is the fix?
   ...
   *Why not C:* Programmatic stage already has a tiered ask menu and named owner in place; what's missing here is any tracking or distribution at all, the defining Reactive symptom, not a reward that needs resizing.
...
3. A company builds a formal, tiered ask menu with rewards scaled by effort, and assigns a named owner who runs a quarterly nomination cycle. However, advocacy requests are still only ever generated when marketing has a specific need, never triggered by renewal or usage milestones. What stage does this represent?
   - **A.** Programmatic; it lacks the lifecycle instrumentation that would move it to Embedded.
   - **B.** Embedded; a tiered ask menu and named owner are the only requirements for this stage.
   - **C.** Reactive; a tiered ask menu doesn't count toward any stage above the baseline.
   - **D.** Identified; a named owner alone qualifies a programme for this stage.
...
4. A company has a well-regarded advocacy programme, but no one tracks whether references actually influence deal outcomes or contribute to pipeline. During a budget review, the programme's funding is cut. What does the model suggest could have prevented this?
   - **A.** Tracking reference-assisted win rate and pipeline contribution from the outset.
   - **B.** Nothing; advocacy programmes are inherently vulnerable to budget cuts.
   - **C.** Focusing exclusively on high-effort asks, since those are self-evidently valuable.
   - **D.** Moving ownership from a dedicated advocacy role to Customer Success.
...
5. A company has an active, tiered advocacy programme that has been running successfully for eighteen months. The team has not re-audited it since it launched. What does the model recommend, and why?
   - **A.** Re-audit at least annually, to catch quiet regression toward Reactive early.
   - **B.** No further action; Programmatic and Embedded status is permanent once reached.
   - **C.** Rebuild the programme entirely every eighteen months, regardless of performance.
   - **D.** Re-audit only if advocacy participation numbers visibly decline first.
