...
7. **Test for clarity:** Ensure the statement is simple enough to be conveyed via word-of-mouth and understood in a brief interaction. Read it aloud to someone outside the company (a friend, a new hire in another department) and ask them to repeat it back in their own words; if they cannot, simplify further.
...
1. **Profile Your Customer:**
   ...
   - **Pains:** What obstacles, frustrations, or risks prevent them from achieving their jobs? Separate pains by severity: a mild annoyance and a business-critical risk both count as "pains" but deserve very different weight in your value map.
...
2. **Map Your Value Proposition:**
   ...
   - **Pain Relievers:** Which of your features directly address customer pains? How? For each, write the mechanism, not just the claim: "automated invoice reminders" is a feature; "removes the two hours per week finance spends chasing late payments" is a pain reliever.
   - **Gain Creators:** Which of your features create outcomes customers want? How? Apply the same mechanism discipline as pain relievers.
...
3. **Analyse the Fit:**
   ...
   - Are there features you offer that don't address any customer pain or gain? These are noise; consider deprioritising or removing them, or at minimum keeping them out of primary messaging even if they stay in the product.
   - Score the fit for each pain and gain (strong fit, partial fit, no fit) so gaps are visible at a glance rather than buried in prose, and revisit the lowest-scoring items first in the next roadmap cycle.

4. **Refine Messaging:** Prioritise messaging around pain relievers and gain creators that matter most to the segment. Avoid talking about features that don't map to customer needs, even if they were expensive or difficult to build; internal pride in a feature is not a reason to lead with it externally.
...
**Fit Analysis:**
- All major pains are addressed: strong fit on 4 of 4 documented pains
- All major gains are created: strong fit on 5 of 5 documented gains
- No wasted features: every one of the five core features mapped to at least one pain or gain, though the feedback-loop feature scored only a partial fit against the retention gain, since it surfaced problems but did not yet help managers act on them
...
**Pitfalls:**
- **Assuming one canvas fits all segments.** A startup and an enterprise have different jobs, pains, and gains. One value proposition won't work for both. Why it matters: messaging written to satisfy both segments at once tends to default to generic language that undersells the fit for either one. Recovery: build a distinct canvas per segment you actively sell to, and route messaging and sales collateral by segment rather than maintaining one shared version.
- **Putting features in the canvas instead of outcomes.** "Dashboard" is not a pain reliever; "See your hiring funnel at a glance" is. Why it matters: a canvas full of feature names looks complete but tells you nothing about fit, because it never actually tests whether the feature addresses a documented pain or gain. Recovery: for every entry in the value map, require a linked pain or gain from the customer profile; entries with no link get removed or investigated as a possible roadmap gap.
- **Ignoring misalignment.** If 50% of your features don't address customer pains or gains, that's a signal to cut features or refocus on different customers. Why it matters: unmapped features still cost engineering time to maintain and can clutter messaging with claims that don't move the buying decision. Recovery: review the fit analysis at each quarterly product review and treat a growing list of unmapped features as a prioritisation red flag, not a footnote.
...
2. **State the advantage: what does the feature let the customer do that they could not do, or could only do with more effort, before?** Write this as a mechanism, not a restatement of the feature: "a manager can pull a board-ready report in under two minutes without exporting to a spreadsheet first," not "makes reporting easier." If you cannot name a concrete "before versus after" here, the feature may not have a real advantage yet, which is itself a useful finding to bring back to product.
3. **State the benefit: what outcome does the customer get, and can you quantify it?** Push past the first answer, which is usually still functional, by asking "and that means what, for them?" at least twice. "Saves time" is a functional restatement; "gives the finance lead two hours back before each board meeting" is a benefit. Where possible, attach a number sourced from a real customer, not an internal estimate.
4. **Test the benefit against a real buyer priority.** Cross-check the benefit you have written against the buyer's actual stated priorities (from win/loss interviews, discovery call notes, or the Value Proposition Canvas's documented pains and gains). A technically true benefit that no target buyer has ever mentioned caring about is not worth leading with, however satisfying it is to write.
5. **Rank ladders by how much a feature actually differentiates.** Not every feature needs a full FAB treatment in every asset; reserve the complete ladder, and the prominent placement that comes with it, for the handful of features that map to a Message Architecture pillar or a genuinely contested buying criterion. Table-stakes features can stay as a plain feature line in a comparison table.
6. **Write the external line from the benefit, not the feature.** The published sentence should lead with the benefit, use the advantage as the supporting clause, and mention the feature only as the proof: "Close the books two days faster (benefit), because reconciliation runs automatically in the background (advantage) using our real-time ledger sync (feature)," not the feature-first order most spec sheets default to.
...
**Pitfalls:**
- **Stopping at the advantage and calling it a benefit.** "Automatically routes invoices" describes a mechanism, not an outcome the buyer feels or measures; teams often stop here because it already sounds more compelling than the raw feature and mistake that improvement for having reached the benefit rung. Recovery: for every draft benefit, ask "and so what happens for the customer because of that?" one more time than feels necessary; if the answer is still describing what the product does rather than what changes for the buyer, it is still an advantage.
- **Writing a benefit with no evidence behind it.** "Saves hours every week" sounds like a benefit but is an unquantified guess if no customer data backs it; a sceptical buyer, or a competitor's rep, can puncture it in one question. Recovery: require a sourced number, from real customer usage or a reference account, before a quantified benefit ships externally; if no data exists yet, mark the claim as directional internally and prioritise getting one reference customer to measure it before the next campaign.
- **Running the full ladder on every feature in the release notes.** Applying the complete three-rung treatment, with prominent placement, to table-stakes or minor features dilutes attention from the one or two features that genuinely differentiate, and trains readers to skim past all of it. Recovery: rank features by how much they map to a validated Message Architecture pillar or a contested buying criterion first, and reserve the full ladder and its prominent placement for that shortlist; let minor features stay as a plain feature line in a comparison table.
