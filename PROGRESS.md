...
0. **Build out [ENRICHMENT-PLAN.md](ENRICHMENT-PLAN.md) (logged 2026-07-21; build approved).** All 19 candidate frameworks *and methodologies* (VoC programme, Dunford positioning process, message architecture, competitive battlecards, GTM motion model, PLG, conjoint analysis, sales enablement content, a GTM-vs-PMM primer, and more) will be built and added to the system. The priority column in the plan is the build order, not a filter. **Do Part B (content-type vocabulary and templates in CLAUDE.md) before or alongside building the first Part A candidate.** First batch: Message Architecture, Competitive Battlecard, Voice of the Customer, Dunford 10-Step Positioning Process, and the GTM-vs-PMM primer. Stand up a new Category 9 (Sales Enablement) as its three entries are built.
