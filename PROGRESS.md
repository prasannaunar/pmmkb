...
**Current Status:** Knowledge base content complete and consistency-reviewed (28 entries: 27 frameworks and 1 methodology, all British English and quality-gate compliant); the first two ENRICHMENT-PLAN.md candidates were built on 2026-07-21 (Message Architecture, Voice of the Customer Programme); polish and operational integration phase ongoing
...
- Number of entries: 28 ✅ (26 as of 2026-07-19; +2 on 2026-07-21 with Message Architecture in Category 2 and the Voice of the Customer Programme in Category 1. Of the 28, 27 are tagged/treated as frameworks and 1 as a methodology; the 10 existing entries that are really methodologies, per ENRICHMENT-PLAN.md B2, are not yet retagged)
- Entries with all required sections populated for their type: 28/28 ✅ (100%; the two new entries follow the Framework and Methodology templates respectively)
- Entries with specific (named or detailed scenario) examples: 28/28 ✅ (100%)
- Entries within the word target (1000-1500 for frameworks/methodologies): 28/28 ✅ (100%; Message Architecture 1,085 words, VoC Programme 1,114 words)
- Entries with cross-references to other entries: 28/28 ✅ (100%; both new entries carry four See-also links)
- Frameworks with Further Reading sections (established only): 6/6 ✅ (100%)
- Entries with metrics benchmarks or explicit success metrics: 28/28 ✅ (100%; both new entries name explicit success metrics)
- Entries with actionable pitfall recovery steps: 28/28 ✅ (100%; both new entries carry three pitfalls with recovery steps. Note: primers, when added, will not require Example/Pitfalls per CLAUDE.md)
- Entries validated against web sources: 19/28 ✅ (the 19 original frameworks were validated 2026-04-09; the 9 added since, Category 8's 3 pricing frameworks, the Product Differentiation recount, and the 2 entries added 2026-07-21, have not had a separate source-validation pass)
- Attribution accuracy verified: 19/19 of the original set ✅ (100%; 2 errors fixed 2026-04-09); Category 8 and the 2026-07-21 entries are converged-practice, cited inline (Message Architecture: Pragmatic Institute / PMA; VoC: Griffin & Hauser 1993) rather than attributed to a single named originator
- Em dashes in entry content: 0/28 ✅ (fixed 2026-07-19; both new entries written em-dash-free)
- British English spelling compliance: 28/28 ✅ (both new entries written in British English)
...
0. **Build out [ENRICHMENT-PLAN.md](ENRICHMENT-PLAN.md) (logged 2026-07-21; build approved; 2 of 19 built).** The first two "High" candidates are done: A11 Message Architecture (Category 2) and A6 Voice of the Customer Programme (Category 1). The remaining 17 candidate frameworks *and methodologies* (Dunford positioning process, competitive battlecards, GTM motion model, PLG, conjoint analysis, sales enablement content, a GTM-vs-PMM primer, and more) will be built and added to the system. The priority column in the plan is the build order, not a filter. Remaining first batch: Competitive Battlecard, Dunford 10-Step Positioning Process, and the GTM-vs-PMM primer (which exercises the Primer template not yet used). Stand up a new Category 9 (Sales Enablement) as its three entries are built. Still outstanding from Part B: the `**Type:**` backfill on the original 26 entries and the Type-column rollout across all INDEX.md and README.md tables (the two new entries are typed in their files and inline in INDEX.md).
