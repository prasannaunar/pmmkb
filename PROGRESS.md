...
## ENRICHMENT-PLAN.md Build: Seventeenth and Eighteenth Candidates (Logged 2026-07-26)
...
- **A15 Gabor-Granger Method** built as Category 8's fifth entry (`frameworks/08-pricing-packaging.md`, 1,510 words, Methodology template: What it is → When to use it → How to run it → Cadence & ownership → Example → Pitfalls → See also). Positioned explicitly as the decisive next step after Van Westendorp PSM: PSM finds the acceptable price corridor, Gabor-Granger narrows that corridor to the single number that maximises expected revenue by multiplying price against a demand curve built from purchase-intent responses at a sequence of price points. Differentiated from Conjoint Analysis by scope (one whole-product price, not a per-feature bundling decision) and cost/speed (a 1-2 week survey versus Conjoint's 6-10 week study).
- **A16 Usage-Based / Consumption Pricing Model** built as Category 8's sixth entry (`frameworks/08-pricing-packaging.md`, 1,518 words, Model template: What it is → When to use it → How to read it → How to apply it → Example → Pitfalls → See also). Framed as strictly downstream of the existing Value Metric/WTP Framework: that framework chooses and validates the consumption unit; this model describes how the bill is structured against that unit (pure usage, hybrid platform-fee-plus-overage, or prepaid credits), read as a spectrum trading off revenue predictability against value alignment rather than a single fixed structure. Cross-referenced to the GTM Motion Model, since self-serve and PLG motions frequently pair with usage-based entry pricing to lower adoption friction.
- Ran the cross-doc consistency checklist for both: INDEX.md (rows added with Type inline; counts 42 → 44; Cat 8 4 → 6; Structure Notes, Completion Status, and Next Steps updated), README.md (headline "44 entries", summary table, version block, new-additions line), FRAMEWORK-MAP.md (both added to the Pricing & Packaging dependency table; Gabor-Granger inserted as an optional step 3a of the Pricing a New Tier or Service scenario, after Van Westendorp PSM; Usage-Based Pricing Model inserted as an optional step 4a, after Value Metric/WTP; version history), BY-SITUATION.md (both inserted into Situation 6 as steps 3a and 4a), ENRICHMENT-PLAN.md (both marked ✅ Built; summary table and status header updated to show the "Lower" batch five-sixths built, leaving only A19), and this file.
- Total entries: 42 → 44 (33 frameworks, 8 methodologies, 2 models, 1 primer). This leaves one "Lower"-priority candidate, A19 Sales Enablement Maturity Model, whose build also requires a decision on whether to stand up the planned Category 9 (Sales Enablement) alongside A17 (Category 3) and A18 (Category 5).
...
- **2026-07-26 (build, seventeenth and eighteenth)**: Built the seventeenth and eighteenth [ENRICHMENT-PLAN.md](ENRICHMENT-PLAN.md) candidates, continuing the "Lower" batch. **A15 Gabor-Granger Method** added to Category 8 (`frameworks/08-pricing-packaging.md`, 1,510 words) using the Methodology template (What it is → When to use it → How to run it → Cadence & ownership → Example → Pitfalls → See also); positioned as the decisive next step after Van Westendorp PSM, narrowing its acceptable price corridor down to the single revenue-maximising number via a demand curve built from purchase-intent responses across a sequence of price points. **A16 Usage-Based / Consumption Pricing Model** added to Category 8 (`frameworks/08-pricing-packaging.md`, 1,518 words) using the Model template (What it is → When to use it → How to read it → How to apply it → Example → Pitfalls → See also); framed as strictly downstream of the Value Metric/WTP Framework, describing how the bill is structured (pure usage, hybrid, or flat) against the value metric that framework chooses. Ran the cross-doc consistency checklist: INDEX.md (Category 8 4 → 6; counts 42 → 44), README.md (headline "44 entries", summary table, version block, new-additions line), FRAMEWORK-MAP.md (both added to the Pricing & Packaging dependency table; Gabor-Granger inserted as an optional step 3a and Usage-Based Pricing Model as an optional step 4a of the Pricing a New Tier or Service scenario; version history), BY-SITUATION.md (both inserted into Situation 6 as steps 3a and 4a), ENRICHMENT-PLAN.md (both marked ✅ Built; summary table and status header updated to show the "Lower" batch five-sixths built), and this file. Total entries: 42 → 44 (33 frameworks, 8 methodologies, 2 models, 1 primer). This leaves one ENRICHMENT-PLAN.md candidate, A19 Sales Enablement Maturity Model, and the Category 9 decision that comes with it.
